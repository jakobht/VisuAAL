...
public class Heatmap extends Simulation implements Serializable {
    ...
    private static SimulationPoint createNewSimulationPoint(SimulationPoint sp) {
        switch (sp.getType()) {
            case EdgePoint:
                return new SimulationEdgePoint((SimulationEdgePoint) sp);
            case NodePoint:
                return new SimulationNodePoint((SimulationNodePoint) sp);
            case Variable:
                return new SimulationPoint(sp);
            default:
                throw new IllegalArgumentException("Heat maps can only edge, node and variable types");
        }
    }
